...
void log_fuse_context(struct fuse_context *context)
{
    ...
    log_struct(((struct asmfs_state *)context->private_data), logfile, %08x, );
    log_struct(((struct asmfs_state *)context->private_data), rootdir, %s, );

    log_struct(((struct asmfs_state *)context->private_data), remotehostname, %s, );
    log_struct(((struct asmfs_state *)context->private_data), remoteUser, %s, );
